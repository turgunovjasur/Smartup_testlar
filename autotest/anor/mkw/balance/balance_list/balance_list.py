...
class BalanceList(BasePage):
    # ------------------------------------------------------------------------------------------------------------------
    header = (By.XPATH, '//button[@ng-if="fi.detail"]')
    ...
    def find_row(self, product_name, warehouse_name):
        self.find_row_and_click(element_name=product_name,
                                xpath_pattern=f'//b-grid[@name="table"]//div[contains(.,"{warehouse_name}") and @class="tbl-cell"]/../div[contains(.,"{product_name}")]/../div/div')
    # ------------------------------------------------------------------------------------------------------------------
