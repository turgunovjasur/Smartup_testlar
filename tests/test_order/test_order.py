...
# Order Add_b ----------------------------------------------------------------------------------------------------------
def test_order_add_action(driver):

    base_page = BasePage(driver)
    base_page.logger.info("Test run(▶️): test_order_add_action")
    ...
    try:
        ...
        assert total_amount_margin == total_sum, base_page.logger.error(f"{total_amount_margin} != {total_sum}")
        ...
        assert get_quantity == product_quantity, base_page.logger.error(f"{get_quantity} != {product_quantity}")
        assert get_price == total_amount, base_page.logger.error(f"{get_price} != {total_amount}")
        assert get_total_sum == total_sum, base_page.logger.error(f"{get_total_sum} != {total_sum}")

        order_view.click_close_button()
        base_page.logger.info(f"✅Test end(): test_order_add_action")
...
def test_order_edit_A(driver):
    ...
    try:
        ...
        order_view.click_close_button()
        base_page.logger.info(f"✅Test end(): test_order_edit_A")
...
def test_order_edit_C(driver):
    # Test data
    base_page = BasePage(driver)
    base_page.logger.info("Test run(▶️): test_order_edit_C")
    ...
    try:
        ...
        order_add_final = OrderAddFinal(driver)
        assert order_add_final.element_visible(), base_page.logger.error('OrderEditFinal not open!')
        get_booked_payment_allowed = order_add_final.get_booked_payment_allowed()
        print(f"get_booked_payment_allowed: {get_booked_payment_allowed}")
        # order_add_final.click_save_button()
        #
        # # Orders List
        # assert order_list.element_visible(), base_page.logger.error('OrdersList not open!')
